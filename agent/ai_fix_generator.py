...
class AIFixGenerator:
    ...
    def generate(self, component: str) -> str:
        ...
        prompt = tmpl.render(audit_results=self.audit, structure=self.struct, component=component)
    ...
    def run_all(self, out_dir: str = "fixes"):
        ...
        for comp in missing:
            if comp == "docker":
                ...
                p = Path(out_dir) / "Dockerfile"
            elif comp == "mlflow":
                content = self.generate("mlflow.yaml")
                p = Path(out_dir) / "mlflow.yaml"
            elif comp == "tests":
                content = self.generate("pytest unit tests")
                p = Path(out_dir) / "tests" / "test_example.py"
